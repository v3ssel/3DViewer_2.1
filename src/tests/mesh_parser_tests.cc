...
TEST(MeshParser, Cube) {
  QString path = QString::fromStdString(
      (std::filesystem::current_path() / "tests/OBJ/cube.obj").string());
  ...
  for (auto it = 0; it < mesh->vertices.size(); ++it) {
    EXPECT_NEAR(mesh->vertices[it].x(), vertices[it].x(), 1e-4);
    EXPECT_NEAR(mesh->vertices[it].y(), vertices[it].y(), 1e-4);
    EXPECT_NEAR(mesh->vertices[it].z(), vertices[it].z(), 1e-4);
  }
  ...
  QVector<QVector3D> normals = {QVector3D(0, 0, 0),  QVector3D(0, 1, 0),
                                ...
                                QVector3D(0, 0, -1)};
  EXPECT_EQ(mesh->normals.size(), normals.size());
  for (auto it = 0; it < mesh->normals.size(); ++it) {
    EXPECT_NEAR(mesh->normals[it].x(), normals[it].x(), 1e-4);
    EXPECT_NEAR(mesh->normals[it].y(), normals[it].y(), 1e-4);
    EXPECT_NEAR(mesh->normals[it].z(), normals[it].z(), 1e-4);
  }
...
TEST(MeshParser, NonExists) {
  EXPECT_THROW(
      s21::Controller::Instance().ParseMeshFromFile("/some/nonexisting/path"),
