...
void Scene::InitModel(Mesh* mesh) {
  ...
  vbo.bind();
  vbo.allocate(mesh_->facets.data(),
               sizeof(mesh_->facets[0]) * mesh_->facets.size());

  ebo.bind();
  ebo.allocate(mesh_->indices.data(),
               sizeof(mesh_->indices[0]) * mesh_->indices.size());
}

void Scene::ResetModel() { mesh_ = nullptr; }
...
void Scene::paintGL() {
  ...
  QVector3D linesColor(lines_color.red() / 255.0f, lines_color.green() / 255.0f,
